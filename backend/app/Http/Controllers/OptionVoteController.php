...
class OptionVoteController extends Controller
{
    ...
    {
        $data = array();
        $optionVotes = OptionVote::where("post_id", $post_id)->where("option_id", $option_id)->get();
    ...
    public function checkHasVoted($post_id, $user_id)
    {
        $option_id = array(1, 2, 3, 4);
        $response = false;

        foreach ($option_id as $value) {
            $optionVote = OptionVote::where("post_id", $post_id)
    ...
    public function deleteOptionVote(Request $request)
    {
        ...
        $optionVote = OptionVote::where("post_id", $request->post_id)
